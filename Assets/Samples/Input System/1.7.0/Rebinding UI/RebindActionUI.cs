...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            {
                foreach (var otherBindingIndex in Enumerable.Range(0, otherAction.bindings.Count))
                {
                    var otherBinding = otherAction.bindings[otherBindingIndex];
                    if (otherBinding.overridePath == bindingToReset.path)
                    {
                        otherAction.ApplyBindingOverride(otherBindingIndex, oldOverridePath);
                    }
                }
            }
        }
        ...
        private bool IsBindingAlreadyInUse(InputAction actionToRebind, int bindingIndex, string newPath)
        {
            var currentBinding = actionToRebind.bindings[bindingIndex];

            foreach (var action in actionToRebind.actionMap.actions)
            {
                for (int i=0; i<action.bindings.Count; i++)
                {
                    var path = action.bindings[i].effectivePath.Replace("<", "/").Replace(">", "");
                    if (action == actionToRebind && action.bindings[i].isPartOfComposite && currentBinding.isPartOfComposite && action.bindings[i].groups == currentBinding.groups)
                    {
                        continue;
                    }
                    if (path == newPath)
        ...
        private bool CheckDuplicateBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            foreach (InputBinding binding in action.actionMap.bindings)
            {
                ...
                var path = binding.effectivePath.Replace("<", "/").Replace(">", "");
                if (path == newBinding.effectivePath) return true;

                if (allCompositeParts)
                {
                    for(int i=0; i<bindingIndex; i++)
                    {
                        var effectivePath = action.bindings[i].effectivePath.Replace("<", "/").Replace(">", "");
                        if (effectivePath == newBinding.overridePath)
                        {
                            return true;
                        }
                    }
